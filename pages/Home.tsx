...
import { ShieldCheck, ArrowRight, Lock, Zap, Activity, Cloud, Search, CheckCircle2, ShieldAlert, Cpu, Globe, Info, Target, Users, Microscope, Database, RefreshCw, Layers, Terminal, Server, CreditCard, Key, Settings, BarChart3, Landmark } from 'lucide-react';
import HSMBlueprint from '../components/HSMBlueprint';

const LogoCloud: React.FC = () => (
  <div className="not-prose py-16 opacity-40 grayscale hover:grayscale-0 transition-all duration-700">
    <div className="flex flex-wrap justify-center gap-x-12 gap-y-8 items-center text-[10px] font-bold uppercase tracking-[0.4em] text-slate-400">
      <div className="flex items-center gap-2 border-r border-slate-200 pr-12"><Landmark size={14}/> NIST Standards</div>
      <div className="flex items-center gap-2 border-r border-slate-200 pr-12"><ShieldCheck size={14}/> BSI Germany</div>
      <div className="flex items-center gap-2 border-r border-slate-200 pr-12"><Cpu size={14}/> IBM Quantum</div>
      <div className="flex items-center gap-2"><Cloud size={14}/> AWS Nitro</div>
    </div>
  </div>
);
    ...
    <article className="prose prose-slate max-w-none">
      ...
      <section className="pt-6 md:pt-16 mb-20 px-4 md:px-0">
        <div className="max-w-4xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-slate-900 text-white text-[9px] font-bold uppercase tracking-[0.2em] mb-8 shadow-sm">
            ...
            System Status: Operational • Berlin Labs
          </div>
          ...
          <p className="text-xl md:text-2xl text-slate-500 font-serif italic leading-relaxed m-0 max-w-3xl">
            A comprehensive transition platform for post-quantum resilience. RivicQ bridges the gap between legacy cryptographic debt and high-assurance hardware security.
          </p>
      ...
      <section id="scope" className="py-24 scroll-mt-24 px-4 md:px-0">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-500 text-xs md:text-sm mt-3 uppercase tracking-widest font-bold">End-to-End Cryptographic Governance</p>
        </div>
        <EndToEndScope />
      </section>

      {/* 3. Enterprise Infrastructure Focus */}
      <section className="py-24 border-y border-slate-100 bg-slate-50 -mx-8 md:-mx-16 px-8 md:px-16 not-prose">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16 px-4">
            <h2 className="text-3xl font-serif font-bold text-slate-900 m-0">Enterprise Roadmap</h2>
            <p className="text-xs text-blue-600 font-bold uppercase tracking-[0.2em] mt-3">High-Assurance Commercial Deployment</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 px-4 md:px-0">
            <div className="p-8 bg-white rounded-3xl border border-slate-200 shadow-sm">
               <div className="flex items-center gap-3 mb-6">
                 ...
                 <h4 className="text-sm font-bold text-slate-900 m-0 uppercase tracking-widest">Sovereign Architecture</h4>
               </div>
               <ul className="space-y-3 m-0 p-0 text-xs text-slate-500">
                 <li className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-blue-500"></div> Cloud Deployment (Active Development)</li>
                 <li className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div> Kratos Auth & Multi-tenancy Isolation</li>
                 <li className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div> Dedicated PostgreSQL Enterprise Clusters</li>
               </ul>
            </div>
            ...
            <div className="p-8 bg-white rounded-3xl border border-slate-200 shadow-sm">
               <div className="flex items-center gap-3 mb-6">
                 ...
                 <h4 className="text-sm font-bold text-slate-900 m-0 uppercase tracking-widest">Physical Entropy Engine</h4>
               </div>
               <ul className="space-y-3 m-0 p-0 text-xs text-slate-500">
                 <li className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-indigo-500"></div> IBM Q / KIPU Hardware Integration</li>
                 <li className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div> Quantum Entanglement Validation</li>
                 <li className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div> FIPS 140-3 L3 Physical Boundary</li>
               </ul>
            </div>
            ...
            <div className="p-8 bg-white rounded-3xl border border-slate-200 shadow-sm">
               <div className="flex items-center gap-3 mb-6">
                 ...
                 <h4 className="text-sm font-bold text-slate-900 m-0 uppercase tracking-widest">Operational Scaling</h4>
               </div>
               <ul className="space-y-3 m-0 p-0 text-xs text-slate-500">
                 <li className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div> Stripe Infrastructure Billing</li>
                 ...
                 <li className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div> SOC2 & DORA Readiness Audit</li>
               </ul>
            </div>
            ...
            <div className="p-8 bg-slate-900 rounded-3xl border border-slate-800 shadow-xl flex flex-col justify-center items-center text-center">
                ...
                <span className="text-slate-400 text-[10px] uppercase tracking-widest font-bold">Projected Enterprise Launch</span>
                <Link to="/roadmap" className="mt-6 text-white text-xs font-bold uppercase tracking-widest border-b border-blue-500 pb-1 hover:text-blue-400 transition-colors">Detailed Milestones</Link>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-3 gap-12">
          <div className="space-y-6">
            ...
            <p className="text-sm text-slate-600 leading-relaxed m-0">
              Operating within the Leap Berlin Quantum accelerator, we represent the next generation of European deep-tech security.
            </p>
          </div>
          <div className="space-y-6">
            ...
            <p className="text-sm text-slate-600 leading-relaxed m-0">
              We protect organizations whose data must remain confidential for 50+ years, neutralizing "Harvest Now, Decrypt Later."
            </p>
          </div>
          <div className="space-y-6">
            ...
            <h3 className="text-2xl font-serif font-bold text-slate-900 m-0">Physics Roots</h3>
            <p className="text-sm text-slate-600 leading-relaxed m-0">
              Our root-of-trust is anchored in physical hardware and verified quantum entropy, not just algorithmic assumptions.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 text-center not-prose bg-slate-900 text-white rounded-[3rem] md:rounded-[4rem] px-8 md:px-16 mb-24 relative overflow-hidden shadow-2xl mx-4 md:mx-0">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-400 text-base md:text-lg mb-12 max-w-xl mx-auto italic leading-relaxed">
            "Cryptographic resilience is not an upgrade; it is a mandatory requirement for institutional operational viability."
          </p>
